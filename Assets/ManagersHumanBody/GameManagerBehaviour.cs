...
public class GameManagerBehaviour : MonoBehaviour
{
    ...
    void Update()
	{
		...
		for(int i =0; i < buttons.Length; i++)
		{
				...
				if (timeStampRoll <= Time.time && canroll == true)
				{
					...
					else if (rollChance <= infectionChance && rollChance > 25 && InfectionLimit < 1)
					{
						Debug.Log("Infection");
						canroll = false;
						timeStampRoll = Time.time + battleRollCooldown;
    ...
    public void LimbClick(string InjuryType)
    {
        ...
        else if(injuryType =="HeadInfection")
        {
            SceneManager.LoadScene(3);
        }
		else if(injuryType == "ChestInfection")
        {
			SceneManager.LoadScene(5);
        }
		else if(injuryType == "KidneyInfection")
        {
